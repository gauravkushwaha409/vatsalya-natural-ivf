...
interface Props {
  answerdata: IAnswersRoot;
  handleSuggestions: any;
}

const CategoriesChatSuggestion: React.FC<Props> = ({
  answerdata,
  handleSuggestions,
}) => {
  ...
  return (
    ...
    >
      {answerdata?.results[0]?.answers?.map((suggestion, i) => (
        <motion.button
        ...
        >
          {suggestion?.suggestions?.map((sugg) => (
            <button onClick={() => handleSuggestions(sugg?.suggestion)}>
              {sugg?.suggestion}
            </button>
          ))}
        </motion.button>
...
export default CategoriesChatSuggestion;
